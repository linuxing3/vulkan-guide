...
void VulkanEngine::init_vulkan() {
  ...
  auto select_device_err = selector.set_minimum_version(1, 3)
                               // .add_required_extension("VK_KHR_surface")
                               // .add_required_extension("VK_KHR_wayland_surface")
                               .set_required_features_13(features)
                               .set_required_features_12(features12)
                               .set_surface(_surface)
                               .select();

  if (!select_device_err) {
    std::cerr << "Failed to select Vulkan Physical Device. Error: "
              << select_device_err.error().message() << "\n";
  }
  vkb::PhysicalDevice physicalDevice{select_device_err.value()};
